public class ConstructInvigilatorTimetable { //takes in exams that are assigned a timeslot and assigns invigilators to them.
    ...
    private LinkedList<Pair<Timeslot, Integer>> timeslots; //(timeslot,index in timetable array) index + 1 is the index (0 indexing) in graph.
    ...
    private void setTimeslots() {
        ...
        for (int i = 0; i < this.exams.length; i++) {
            if (this.exams[i] == null || this.exams[i].length == 0)
                continue;
            this.timeslots.append(new Pair<>(this.exams[i][0].getTimeslot(), i));
        }
    }
        ...
        if (maxflow.getFlow() < this.numExams) {
            ...
            for (int i = 1; i < this.timeslots.len() + 1; i++) {
                for (int j = this.timeslots.len() + 1; j < this.timeslots.len() + this.invigilators.length + 1; j++) { //every invigilator
                    if (maxflow.getResGraph()[i][j] == 0) { //if there was a flow between timeslot and invigilator, then assign invigilator to an exam in that timeslot
                        for (int k = 0; k < this.exams[i - 1].length; k++) {
                            if (this.exams[i - 1][k].getInvigilator() == null) { //first exam in timeslot that is not assigned an invigilator
                                this.exams[i - 1][k].setInvigilator(invigilators[j - this.timeslots.len() - 1]);
                            }
                        }
                    }
                }
